...
impl Blob {
    pub fn new(namespace: Namespace, data: Vec<u8>) -> Result<Blob> {
        let commitment = Commitment::for_blob(namespace, appconsts::SHARE_VERSION_ZERO, &data[..])?;
    ...
    pub fn validate(&self) -> Result<()> {
        let computed_commitment =
            Commitment::for_blob(self.namespace, self.share_version, &self.data)?;
...
impl TryFrom<RawBlob> for Blob {
    ...
    fn try_from(value: RawBlob) -> Result<Self, Self::Error> {
        ...
        let commitment =
            Commitment::for_blob(namespace, value.share_version as u8, &value.data[..])?;

...
impl LuminaNode {
    ...
    pub fn new(network: Network) -> Result<Arc<Self>> {
        let runtime = Runtime::new().map_err(|e| LuminaError::NetworkError {
            message: e.to_string(),
        })?;
    ...
    pub fn start(&self) -> Result<bool> {
        self.runtime.block_on(async {
            ...
            let p2p_bootnodes =
                canonical_network_bootnodes(self.network.into()).collect::<Vec<_>>();
    ...
    pub fn is_running(&self) -> bool {
        self.runtime
            .block_on(async { self.node.lock().await.is_some() })
    }
    ...
    pub fn peer_tracker_info(&self) -> Result<PeerTrackerInfo> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn wait_connected_trusted(&self) -> Result<()> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn listeners(&self) -> Result<Vec<String>> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn set_peer_trust(&self, peer_id: PeerId, is_trusted: bool) -> Result<()> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
            let peer_id = peer_id
    ...
    pub fn request_head_header(&self) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn request_header_by_hash(&self, hash: String) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn request_header_by_height(&self, height: u64) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn request_verified_headers(
        &self,
    ...
    pub fn syncer_info(&self) -> Result<SyncingInfo> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn get_network_head_header(&self) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn get_local_head_header(&self) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn get_header_by_hash(&self, hash: String) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
            let hash = Hash::from_str(&hash).map_err(|e| LuminaError::InvalidHash {
    ...
    pub fn get_header_by_height(&self, height: u64) -> Result<String> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn get_sampling_metadata(&self, height: u64) -> Result<Option<String>> {
        self.runtime.block_on(async {
            let node_guard = self.node.lock().await;
            let node = node_guard.as_ref().ok_or(LuminaError::NodeNotRunning)?;
    ...
    pub fn events_channel(&self) -> Result<Option<NodeEvent>> {
        self.runtime.block_on(async {
            let mut events_guard = self.events_subscriber.lock().await;

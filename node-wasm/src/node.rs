...
use crate::worker::commands::{
    CheckableResponseExt, NodeCommand, SingleHeaderQuery, WorkerResponse,
};
use crate::worker::{GlooWorker, WorkerError};
...
struct NodeDriver {
    worker: Mutex<ReactorBridge<GlooWorker>>,
}
...
impl NodeDriver {
    ...
    pub async fn new() -> Result<NodeDriver> {
        ...
        Ok(Self {
            worker: Mutex::new(GlooWorker::spawner().spawn("lumina.js")),
        })
    }
    ...
    async fn exec(&self, command: NodeCommand) -> Result<WorkerResponse, WorkerError> {
        let mut worker_lock = self.worker.lock().await;
        worker_lock
            .send(command)
            .await
            .expect("channel to not close");
        worker_lock.next().await.expect("channel to not close")
    }

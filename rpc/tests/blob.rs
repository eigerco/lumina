...
async fn blob_submit_and_get() {
    ...
    let blob = Blob::new(namespace, AppVersion::V1, data).unwrap();
...
async fn blob_submit_and_get_all() {
    ...
    let blobs = &[
        Blob::new(namespaces[0], AppVersion::V1, random_bytes(5)).unwrap(),
        Blob::new(namespaces[1], AppVersion::V1, random_bytes(15)).unwrap(),
    ];
...
async fn blob_submit_and_get_large() {
    ...
    blob.validate(AppVersion::V1).unwrap();
    assert_blob_equal_to_sent(&received_blob, &blob);
...
async fn blob_subscribe() {
    ...
    let blob = Blob::new(namespace, AppVersion::V1, random_bytes(10)).unwrap();
    ...
    let blob_another_ns = Blob::new(random_ns(), AppVersion::V1, random_bytes(10)).unwrap();
    ...
    let blob1 = Blob::new(namespace, AppVersion::V1, random_bytes(10)).unwrap();
    let blob2 = Blob::new(random_ns(), AppVersion::V1, random_bytes(10)).unwrap(); // different ns
    let blob3 = Blob::new(namespace, AppVersion::V1, random_bytes(10)).unwrap();

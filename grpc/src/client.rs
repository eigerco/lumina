...
use crate::grpc::{
    BroadcastMode, ConfigResponse, Context, GasEstimate, GasInfo, GetTxResponse, GrpcCall,
...
impl GrpcClient {
    ...
    fn get_auth_params(&self) -> GrpcCall<AuthParams>;

    /// Get account
    #[grpc_method(AuthQueryClient::account)]
    fn get_account(&self, account: &AccAddress) -> GrpcCall<Account>;

    /// Get accounts
    #[grpc_method(AuthQueryClient::accounts)]
    fn get_accounts(&self) -> GrpcCall<Vec<Account>>;
    ...
    pub fn get_verified_balance(
        ...
        GrpcCall::new(call_fn, self.inner.context.clone())
    }

    /// Retrieves the Celestia coin balance for the given address.
    #[grpc_method(BankQueryClient::balance)]
    fn get_balance(&self, address: &Address, denom: impl Into<String>) -> GrpcCall<Coin>;

    /// Get balance of all coins
    #[grpc_method(BankQueryClient::all_balances)]
    fn get_all_balances(&self, address: &Address) -> GrpcCall<Vec<Coin>>;

    /// Get balance of all spendable coins
    #[grpc_method(BankQueryClient::spendable_balances)]
    fn get_spendable_balances(&self, address: &Address) -> GrpcCall<Vec<Coin>>;

    /// Get total supply
    #[grpc_method(BankQueryClient::total_supply)]
    fn get_total_supply(&self) -> GrpcCall<Vec<Coin>>;

    // cosmos.base.node

    /// Get node configuration
    #[grpc_method(ConfigServiceClient::config)]
    fn get_node_config(&self) -> GrpcCall<ConfigResponse>;

    // cosmos.base.tendermint

    /// Get latest block
    #[grpc_method(TendermintServiceClient::get_latest_block)]
    fn get_latest_block(&self) -> GrpcCall<Block>;

    /// Get block by height
    #[grpc_method(TendermintServiceClient::get_block_by_height)]
    fn get_block_by_height(&self, height: i64) -> GrpcCall<Block>;
    ...
    fn abci_query(
        ...
        height: u64,
        prove: bool,
    ) -> GrpcCall<AbciQueryResponse>;

    // cosmos.tx

    /// Broadcast prepared and serialised transaction
    #[grpc_method(TxServiceClient::broadcast_tx)]
    fn broadcast_tx(&self, tx_bytes: Vec<u8>, mode: BroadcastMode) -> GrpcCall<TxResponse>;

    /// Get Tx
    #[grpc_method(TxServiceClient::get_tx)]
    fn get_tx(&self, hash: Hash) -> GrpcCall<GetTxResponse>;

    /// Broadcast prepared and serialised transaction
    #[grpc_method(TxServiceClient::simulate)]
    fn simulate(&self, tx_bytes: Vec<u8>) -> GrpcCall<GasInfo>;
    ...
    ) -> GrpcCall<QueryDelegationResponse>;
    ...
    fn query_unbonding(
        ...
        delegator_address: &AccAddress,
        validator_address: &ValAddress,
    ) -> GrpcCall<QueryUnbondingDelegationResponse>;
    ...
    ) -> GrpcCall<QueryRedelegationsResponse>;

    // celestia.blob

    /// Get blob params
    #[grpc_method(BlobQueryClient::params)]
    fn get_blob_params(&self) -> GrpcCall<BlobParams>;

    // celestia.core.tx

    /// Get status of the transaction
    #[grpc_method(TxStatusClient::tx_status)]
    fn tx_status(&self, hash: Hash) -> GrpcCall<TxStatusResponse>;
    ...
    fn estimate_gas_price(&self, priority: TxPriority) -> GrpcCall<f64>;
    ...
    /// # }
    /// ```
    pub fn submit_message<M>(&self, message: M, cfg: TxConfig) -> GrpcCall<TxInfo>
    where
        M: IntoProtobufAny + Send + 'static,
    {
        ...
        let call_fn = move |context| {
            async move { this.submit_message_impl(message, cfg, &context).await }.boxed()
        };
    ...
    /// # }
    /// ```
    pub fn submit_blobs(&self, blobs: &[Blob], cfg: TxConfig) -> GrpcCall<TxInfo> {
        let this = self.clone();
        let blobs = blobs.to_vec();

        let call_fn = move |context| {
            async move { this.submit_blobs_impl(&blobs, cfg, &context).await }.boxed()
        };
    ...
    pub fn app_version(&self) -> GrpcCall<AppVersion> {
        let this = self.clone();
    ...
    /// Note that this function _may_ try to load the account information from the network and as
    /// such requires signed to be set up.
    pub fn chain_id(&self) -> GrpcCall<Id> {
        let this = self.clone();
